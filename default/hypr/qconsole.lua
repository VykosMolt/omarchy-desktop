...
-- Seed the console with a terminal the first time it opens, rather than at
-- boot, so nothing is running until it is wanted. The exec rule has to pin the
-- workspace itself: Hyprland only tags a spawn with the workspace it came from
-- while misc.initial_workspace_tracking is on, and looknfeel turns it off.
