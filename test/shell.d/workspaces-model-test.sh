...
# The click must not go back through a login shell.
grep -q 'Hyprland.dispatch' "$WIDGET" || fail "the click dispatches over the socket"
# Comments are stripped: this file explains at length what bar.run used to cost.
! grep -vE '^[[:space:]]*//' "$WIDGET" | grep -q 'bar\.run' ||
  fail "the click does not go through bar.run's login shell"
pass "the click dispatches over the socket the widget already holds"
